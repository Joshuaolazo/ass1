...
int column_finder(char* row, char* column){
	...
	// loop setup
	int position= pointer_tocolumn-row;
	int commas = 0;
	int inquotes = 0;
	int i= 0;
...
char* column_reader(char* row, int column){
	...
	//column_string[strcspn(column_string, "\303" )] = '\0';
	
	
	/*
	 memcpy(column_string, row+start, diff);
	 int leng= strlen(column_string);
	 column_string[leng]='\0';
	 
	 printf("string: %s\n",column_string);
	 int la= strlen(column_string);
	 printf("str1 l is: %d\n" ,la);
